...
2. The reviews that display on product listing pages will be loaded from the "Review Count" and "Rating" product attributes. Those attributes get updated on a nightly basis via a CRON job that runs at 4AM. The CRON job will request an XML feed from TurnTo and will update those product attributes. If you encounter issues where those attributes are not updated like you'd expect, you can manually review the contents of the XML feed that Magento is retrieving from TurnTo by creating a url that matches this pattern: `http://www.turnto.com/static/export/<SITE KEY>/<AUTHORIZATION KEY>/turnto-skuaveragerating.xml` where `<SITE KEY>` and `<AUTHORIZATION KEY>` are the values you entered into the corresponding fields in the TurnTo configuration fields in the admin. The XML file is generated by TurnTo on a nightly basis (at 3AM EST), so once you approve a new review for a product, you'll need to wait until the following day before you'll see it reflected in the XML file.
