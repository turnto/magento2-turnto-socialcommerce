...
use TurnTo\SocialCommerce\Model\Manager\Export\Orders as OrdersExportManager;
...
class OrdersTest extends \PHPUnit\Framework\TestCase
{
     ...
     */
    const TURNTO_AUTH_KEY = 'JLWPk365eMx6pj6mv1eXR823LqzSauth';
    ...
    protected function setUp()
    {
        ...
        $this->ordersExportManager = new OrdersExportManager(
            $config,
     ...
     */
    public function testIsOrderExportClass()
    {
        $this->assertInstanceOf(\TurnTo\SocialCommerce\Model\Manager\Export\Orders::class, $this->ordersExportManager);
    }
    ...
    public function testTransmitFeed()
    {
        ...
        $this->ordersExportManager->transmitFeed($outputHandle, $store);
    }
    ...
    public function testGetOrders()
    {
        $this->assertInstanceOf(
            ...
            $this->ordersExportManager->getOrders(1, $this->mockDateTime(), $this->mockDateTime())
        );
    }
    ...
    public function testFormatOrderData()
    {
        ...
        $this->assertTrue(is_array($this->ordersExportManager->formatOrderData([$this->mockOrder()])));
    }

    /**
     * test the creation of the export file
     */
    public function testGenerateOrdersFeed(){
        $cleanedOutput = trim(preg_replace('/\s+/', ' ', $this->ordersExportManager->generateOrdersFeed('1',[])));
        $this->assertEquals($cleanedOutput,utf8_encode('ORDERID ORDERDATE EMAIL ITEMTITLE ITEMURL FIRSTNAME LASTNAME SKU ITEMLINEID ZIP PRICE ITEMIMAGEURL DELIVERYDATE'));
    ...
    private function mockOrderCollection()
    {
        ...
        $dbSelect = $this->createMock(\Magento\Framework\DB\Select::class, ['joinLeft', 'joinLeft->joinLeft', 'group']);
        $dbSelect->expects($this->any())->method('joinLeft')->willReturn($select);
        $dbSelect->expects($this->any())->method('group')->willReturn($select);

        $orderCollectionMock = $this->createMock(\Magento\Sales\Model\ResourceModel\Order\Collection::class);
        $orderCollectionMock->expects($this->any())->method('getSelect')->willReturn($dbSelect);
    ...
    public function mockShipmentRepo()
    {
        $shipmentRepo = $this->getMockBuilder('\Magento\Sales\Api\ShipmentRepositoryInterface')
            ->disableOriginalConstructor()
            ->setMethods(['getList', 'get', 'delete', 'save', 'create', 'getLastPageNumber', 'getPageSize'])
            ->getMock();
        $shipmentRepo->expects($this->any())->method('getList')->willReturn($this->mockProduct());
        $shipmentRepo->expects($this->any())->method('getLastPageNumber')->willReturn('1');
        $shipmentRepo->expects($this->any())->method('getPageSize')->willReturn('1');
    ...
    public function mockExportHelper(){
        ...
        $shipmentRepo = $this->getMockBuilder('\TurnTo\SocialCommerce\Helper\Export\Order')
            ->disableOriginalConstructor()
            ->setMethods(['getShipmentSearchCriteriaForOrder'])
            ->getMock();
        $shipmentRepo->expects($this->any())->method('getShipmentSearchCriteriaForOrder')->willReturn($searchCriteria);
